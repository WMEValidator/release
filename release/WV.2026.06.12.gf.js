...
var _translations = {
  ...
  'CL': {'.codeISO': 'CL', '.country': 'Chile', '.fallbackCode': 'ES', '.lng': 'ES-CL'},
  'CH': {
	'.codeISO': 'CH',
	'.country': 'Switzerland',
	'59.enabled': true,
...
function getMsg(mType, msg, newLine) {
  return 'Validator' + (mType ? ' ' + mType : '') + (msg ? ':' + (newLine ? '\n' : ' ') + msg : '')
}
function log(msg) {
...
function checkFilter(severity, objectCopy, seenObjects) {
  ...
  if (objectCopy.$userName !== _RT.$topUser.$userName && !_UI.pMain.pSearch.oIncludeYourEdits.NODISPLAY && _UI.pMain.pSearch.oIncludeYourEdits.CHECKED) return false;
...
function onObjectsRemoved(e) {
  if (e.dataModelName == 'segments') {
	...
	if (1 === e.objectIds.length)
	  if (RTStateIs(ST_STOP) || RTStateIs(ST_PAUSE)) sync(F_ONSEGMENTSCHANGED, e.objectIds)
  } else if (e.dataModelName == 'venues') {
	_RT.$isMapChanged = true;
	if (1 === e.objectIds.length)
	  if (RTStateIs(ST_STOP) || RTStateIs(ST_PAUSE)) sync(F_ONVENUESCHANGED, e.objectIds)
  } else if (e.dataModelName == 'nodes') {
	_RT.$isMapChanged = true;
	if (1 === e.objectIds.length)
	  if (RTStateIs(ST_STOP) || RTStateIs(ST_PAUSE)) sync(F_ONNODESCHANGED, e.objectIds)
  }
}
function onObjectsAdded(e) {
  if (e.dataModelName == 'segments' || e.dataModelName == 'venues') _RT.$isMapChanged = true
}
...
function F_SHOWREPORT(reportFormat) {
  ...
  function getTopPermalink() {
	...
	return window.location.origin + window.location.pathname + '?zoomLevel=' + zoom + '&lat=' + Math.round(center.lat * 1E5) / 1E5 + '&lon=' + Math.round(center.lon * 1E5) / 1E5 +
  ...
  function getListOfChecks(countryID, country) {
	...
	let ret = trS('report.list.see') + ' ' + Bb + trS('report.list.checks') + Eb + Br + Br;
	let fallbacks = '';
	if (ccode)
  ...
  switch (reportFormat) {
	...
	case RF_CREATEPACK:
	  ...
	  if (!window.confirm(getMsg(
			  ...
			  true)))
		break;
	  country = _RT.$topCity && _RT.$topCity.$country ? _RT.$topCity.$country : window.prompt(getMsg(wType, '\nWME country name (example: United Kingdom):', true));
	  if (!country) break;
	  var ucountry = country.toUpperCase();
...
function F_VALIDATE(disabledHL) {
  ...
  function SimpleNODE(objID, segID) {
	...
	this.$rawNode = n;
	this.rNode = null;
	if (objID) this.rNode = wmeSDK.DataModel.Nodes.getById({nodeId: objID});
	if (n) {
	  ...
	  for (let i = 0; i < this.rNode?.connectedSegmentIds.length; i++) {
		...
		if (+segID === +si || !wmeSDK.DataModel.Segments.getById({segmentId: si})) continue;
  ...
  SimpleNODE.prototype.getOutConnections = function() {
	return this._outConnections ? this._outConnections : this._outConnections = this._rawOutConnections.map(function(e) {
	  return new SimpleOBJECT(e, 'segment')
	})
  };
  ...
  SimpleRESTRICTION.prototype.getTo = function() {
	return this._to ? this._to : this._to = new SimpleOBJECT('' + this.$toID, 'segment')
  };
  function SimpleOBJECT(rawobj, obtype) {
	...
	if (this.$objtype === 'segment') {
	  ...
	  if (raw.hasOwnProperty('length')) this.$length = raw.length;
	  if (this.$length < 7) this.$length = turf.length(raw.geometry, u_meters);
	  ...
	  if (this.$isParkingLot) {
		const parkingType = wmeSDK.DataModel.Venues.ParkingLot.getParkingLotType({venueId: this.$objectID});
		const costType = wmeSDK.DataModel.Venues.ParkingLot.getCostType({venueId: this.$objectID});
		const lotType = wmeSDK.DataModel.Venues.ParkingLot.getLotTypes({venueId: this.$objectID});
		const paymentType = wmeSDK.DataModel.Venues.ParkingLot.getPaymentMethods({venueId: this.$objectID});
		this.$parkAttr = { parkingType, costType, lotType, paymentType }
	  }
	  this.$categoryAttributes = '';
	  this.$isGasStation = false;
	  this.$categories.forEach((c, i) => {
		if (c == 'GAS_STATION') this.$isGasStation = true
	  });
	...
	if (mod.updatedOn) this.$updatedOn = formatDate(mod.updatedOn);
	if (mod.updatedBy) this.$updatedByName = mod.updatedBy;
	if (mod.createdOn) this.$createdOn = formatDate(mod.createdOn);
	if (mod.createdBy) this.$createdByName = mod.createdBy;
	Object.defineProperties(this, {
  ...
  SimpleOBJECT.prototype.getRestrictions = function() {
	var t;
	return this._restrictions ? this._restrictions : this._restrictions = this.$objtype == 'venue' ? [] : (t = this, this.$rawObject.restrictions.map(function(e) {
  ...
  SimpleOBJECT.prototype.report = function(params) {
	...
	function getObjectCopy(ss) {
	  return {
		$objectID: ss.$objectID, $objtype: ss.$objtype, $name: ss.$name, $countryID: +ss.$address.$countryID, $cityID: +ss.$address.$cityID, $streetID: +ss.$address.$streetID, $reportIDs: {},
			$updated: ss.$updatedOn ? ss.$rawObject.modificationData.updatedOn :
			ss.$createdOn			? ss.$rawObject.modificationData.createdOn :
									  0,
			$userName: ss.$updatedByName ? ss.$updatedByName :
			ss.$createdByName			 ? ss.$createdByName :
										   '',
  ...
  function addHLedObjects() {
	...
	for (let i in _RT.$HLedObjects) {
	  ...
	  if (obj.$severity) wmeSDK.Map.addFeatureToLayer({feature: {type: 'Feature', id: i, geometry: obj.$geometry, properties: {type: obj.$severity}}, layerName: GL_LAYERNAME})
	}
  }
  ...
  function matchRegExp(checkID, objectID, expandedString, loptions) {
	...
	if (loptions[CO_NUMBER] && 0 < _REP.$debugCounter) {
	  var checkTitle = '';
  ...
  let sel;
  try {
	sel = wmeSDK.Editing.getSelection()
  } catch (e) {
	sel = null;
  ...
  for (let r in segs) {
	...
	if (rawSegment.fromNodeId == null && rawSegment.toNodeId == null) continue;
	var seen = null;
	if (segmentID in _RT.$seen) seen = _RT.$seen[segmentID];
	let isSel = false;
	for (let i = 0; i < selectedObjects.length; i++)
	  if (selectedObjects[i] == segmentID) {
		isSel = true;
		break
	  }
	if (isSel) {
	...
	var segmentGeometry = rawSegment.geometry;
	if (seen) {
	...
	if (streetLen && isDrivable && address.isOkFor(101)) {
	...
	if (RT_RAILROAD === roadType && 100 > segmentLen && streetLen > 0 && !isPartial && !nodeA.$otherSegmentsLen && !nodeB.$otherSegmentsLen && address.isOkFor(104)) segment.report(104);
	...
	if (slowChecks) {
	  if (1 === nodeA.$otherSegmentsLen && DIR_UNKNOWN !== direction && !nodeA.$isPartial && !nodeA.$isUturn && !nodeA.$restrictionsLen && !segment.$restrictionsLen && !hasClosures && isLimitOk(36) &&
		...
		if ((!nodeB.$isPartial || !nextNode.$isPartial) && otherSegment.$segmentID !== segmentID && otherSegment.$rawObject && (1E4 > otherSegment.$length + segmentLen || 1E3 > segmentLen) &&
	  ...
	  if (DIR_UNKNOWN !== direction && !nodeB.$isPartial && 1 === nodeB.$otherSegmentsLen && !nodeB.$isUturn && !nodeB.$restrictionsLen && !segment.$restrictionsLen && !hasClosures && isLimitOk(37) &&
		...
		if ((!nodeA.$isPartial || !nextNode.$isPartial) && otherSegment.$segmentID !== segmentID && otherSegment.$rawObject && 1E4 > otherSegment.$length + segmentLen &&
			otherSegment.$address.$street === street && otherSegment.$address.$city === city && otherSegment.$address.$state === state && otherSegment.$address.$country === country &&
			otherSegment.$fwdMaxSpeed === forwardSpeed && otherSegment.$revMaxSpeed === reverseSpeed && otherSegment.$fwdMaxSpeedUnverified === forwardSpeedUnverified &&
			otherSegment.$revMaxSpeedUnverified === reverseSpeedUnverified && otherSegment.$type === roadType && otherSegment.$isToll === isToll && otherSegment.$hasRestrictions === hasRestrictions &&
			!otherSegment.$hasClosures && deepCompare(otherSegment.$flags, flags) &&
	...
	if (cityLen) {
	  ...
	  if (RT_RAILROAD === roadType && isLimitOk(27) && address.isOkFor(27)) segment.report(27);
	...
	if (isDrivable) {
	  ...
	  if (!nodeA.$isPartial) {
		if (slowChecks && 5 < segmentLen && !nodeA.$otherSegmentsLen && nodeA.getCenter() && isLimitOk(107) && address.isOkFor(107)) {
		  ...
		  for (let r in segs) {
			...
			if (seg.fromNodeId == null && seg.toNodeId == null) continue;
			if (RR_TRAIL >= SimpleOBJECT.prototype.getTypeRank(seg.roadType)) continue;
			if (LIMIT_TOLERANCE > turf.pointToLineDistance(nPt, seg.geometry, u_meters)) {
			  if (_RT.$topUser.$userLevel <= seg.lockRank) segment.$forceNonEditable = true;
			  segment.report(107);
			  break
			}
		  }
		...
		if (!nodeB.$isPartial) {
		  ...
		  if (slowChecks && 15 > segmentLen && !streetLen && 2 === nodeA.$otherSegmentsLen && 2 === nodeB.$otherSegmentsLen && isLimitOk(79) && address.isOkFor(79) &&
			  nodeA.$otherSegments[0].$rawObject && nodeA.$otherSegments[1].$rawObject && nodeB.$otherSegments[0].$rawObject && nodeB.$otherSegments[1].$rawObject &&
	  ...
	  if (!nodeB.$isPartial) {
		if (slowChecks && 5 < segmentLen && !nodeB.$otherSegmentsLen && nodeB.getCenter() && isLimitOk(108) && address.isOkFor(108)) {
	...
	if (streetLen) {
	  let checkIDType = {160: RT_FREEWAY, 161: RT_MAJOR, 162: RT_MINOR, 163: RT_RAMP, 164: RT_PRIMARY, 165: RT_STREET, 166: RT_PARKING, 167: RT_RAILROAD, 169: 0};
  ...
  if (_UI.pMain.pFilter.oEnablePlaces.CHECKED) {
	...
	for (let i = 0; i < vlist.length; i++) {
	  ...
	  } else if (venueID in _RT.$revalidate) {
		deleteSeenObject(venueID);
		seen = null;
	  ...
	  let countryCode = country ? _I18n.getCountryCode(country.toUpperCase()) : _RT.$cachedTopCCode;
	  ...
	  if (isLimitOk(252)) {
		...
		if (options[CO_REGEXP].test(venue.$updatedBy.toString()) && address.isOkFor(252)) venue.report(252)
	  }
	  ...
	  if (venue.$entryExitPoints && venue.$entryExitPoints.length && isLimitOk(254)) {
		...
		if (areaCenter.geometry && turf.booleanEqual(areaCenter.geometry, stopPoint) && address.isOkFor(254)) venue.report(254)
	  }
...
function addPanelDetails() {
  const selection = wmeSDK.Editing.getSelection();
  if (selection && selection.ids.length > 0 && (selection.objectType == 'segment' || selection.objectType == 'venue')) updateObjectProperties(selection.ids, false)
}
function updateObjectProperties(selectedObjects, disabledHL) {
  ...
  selectedIssues.forEach(function(e) {
	...
	var shortTitle = exSOS(check.TITLE, xoptions, 'titleEN').replace('WME Color Highlights', 'WMECH').replace('WME Toolbox', 'WMETB');
	newProp += '<div class="c' + CL_RIGHTTIP + ' c' + sevClass + '">' +
		...
		'<div class="c' + CL_RIGHTTIPDESCR + '">' +
		'#' + checkID + ' ' + exSOS(check.PROBLEM, xoptions, 'problemEN');
	var pl = trO(check.PROBLEMLINK, ccode);
	if (pl)
...
async function F_LOGIN() {
  ...
  _RT = {
	...
	$topCity: null,
	$cachedTopCCode: '',
	$topUser: {$userName: wmeSDK.State.getUserInfo().userName, $userLevel: wmeSDK.State.getUserInfo().rank + 1},
	$topCenter: null,
	$WDmoveID: -1,
	$WDloadID: -1,
	layerToggle: [],
  ...
  listOfPacks += '<br><b>See</b> <a target="_blank" href="' + PFX_DISCUSS + DISCUSS_LOCAL + '">' +
  ...
  for (let i = 1; i < MAX_CHECKS; i++) {
	...
	for (let ccode in _I18n.$translations) {
	  ...
	  if (labelPL in translation) {
		let l = translation[labelPL].replace('W:', PFX_WIKI).replace('P:', PFX_PEDIA).replace('F:', PFX_FORUM).replace('D:', PFX_DISCUSS);
	  ...
	  if (labelSL in translation) {
		let l = translation[labelSL].replace('W:', PFX_WIKI).replace('P:', PFX_PEDIA).replace('F:', PFX_FORUM).replace('D:', PFX_DISCUSS);
  ...
  if (!classCodeDefined(window.AudioContext) && !classCodeDefined(window.webkitAudioContext)) {
	_UI.pSettings.pScanner.oSounds.CHECKED = false;
  ...
  const styleRules = [
	{style: {strokeWidth: HL_WIDTH}}, {predicate: properties => {return properties.type == RS_NOTE}, style: {strokeColor: GL_NOTECOLOR, graphicZIndex: 10}},
	{predicate: properties => {return properties.type == RS_WARNING}, style: {strokeColor: GL_WARNINGCOLOR, graphicZIndex: 20}},
	{predicate: properties => {return properties.type == RS_ERROR}, style: {strokeColor: GL_ERRORCOLOR, graphicZIndex: 30}}
  ];
  ...
  $(tabLabel.parentElement).prepend($('<span>', {class: 'fa fa-check-square-o'}));
...
function F_ONNODESCHANGED(e) {
  ...
  for (let i = 0; i < e.length; i++) {
	...
	if (nd) {
	  ...
	  for (let j = 0; j < ids.length; j++) _RT.$revalidate[ids[j]] = true, reHL = true
	}
  }
...
function F_ONCHANGELAYER(e) {
  if (!e.hasOwnProperty('layerName')) return;
  if (GL_LAYERNAME === e.layerName && wmeSDK.Map.isLayerVisibile({layerName: GL_LAYERNAME}) !== _UI.pSettings.pScanner.oHLReported.CHECKED) {
...
function F_ONMOVEEND() {
  ...
  if (-1 === _RT.$WDmoveID && -1 === _RT.$WDloadID && ptEqual(c, _RT.$nextCenter))
	_RT.$WDmoveID = window.setTimeout(onMergeEnd, WD_SHORT);
  else if (RTStateIs(ST_RUN) && !_RT.$firstStep && !ptEqual(c, _RT.$nextCenter) && !ptEqual(c, _RT.$startCenter)) {
...
function F_ONLOADSTART() {
  const c = wmeSDK.Map.getMapCenter();
  window.clearTimeout(_RT.$WDmoveID);
  if (-1 === _RT.$WDloadID && ptEqual(c, _RT.$nextCenter)) _RT.$WDloadID = window.setTimeout(onMergeEnd, WD_LONG);
  _RT.$WDmoveID = -1
}
...
function F_LAYERSON() {
  if (_RT.layerToggle.length == 0 || GL_SHOWLAYERS) return;
...
function F_STOP() {
  if (!RTStateIs(ST_STOP)) {
	beep(100, 'square');
	if (_RT.$startCenter) wmeSDK.Map.setMapCenter({lonLat: _RT.$startCenter, zoomLevel: _RT.$startZoom});
...
function F_ONMERGEEND() {
  ...
  const c = wmeSDK.Map.getMapCenter();
  if (RTStateIs(ST_RUN) && _RT.$nextCenter && !ptEqual(c, _RT.$nextCenter)) return;
...
function F_INIT() {
  ...
  setupPolicy();
  const panelObserver = new MutationObserver(mutations => {mutations.forEach(function(mutation) {
											   for (let i = 0; i < mutation.addedNodes.length; i++) {
												 const addedNode = mutation.addedNodes[i];
												 if (addedNode.nodeType === Node.ELEMENT_NODE)
												   if (addedNode.querySelector('.address-edit')) addPanelDetails()
											   }
											 })});
  ...
  _WV.SimpleADDRESS = function(objID) {
	...
	if (objID?.isEmpty)
	  ...
	  this.$streetID = objID;
	if (this.$streetID) {
	  let o = wmeSDK.DataModel.Streets.getById({streetId: this.$streetID});
	  if (o) {
...
window.SDK_INITIALIZED.then(() => {
  ...
  wmeSDK.Events.once({eventName: 'wme-ready'}).then(async () => {F_INIT()})
});
var _I18n = {
  $defLng: 'EN',
  $lng: '',
