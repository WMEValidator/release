...
function checkFilter(severity, segmentCopy, seenSegments) {
  if (seenSegments) {
	if (segmentCopy.$segmentID in seenSegments && _UI.pMain.pFilter.oExcludeDuplicates.CHECKED) return false;
	seenSegments[segmentCopy.$segmentID] = null
  }
  if ((RR_STREET === segmentCopy.$typeRank || RR_SERVICE === segmentCopy.$typeRank) && _UI.pMain.pFilter.oExcludeStreets.CHECKED) return false;
  if (RR_SERVICE > segmentCopy.$typeRank && _UI.pMain.pFilter.oExcludeOther.CHECKED) return false;
  if (!segmentCopy.$isEditable && _UI.pMain.pFilter.oExcludeNonEditables.CHECKED) return false;
  if (RS_NOTE === severity && _UI.pMain.pFilter.oExcludeNotes.CHECKED) return false;
  if (segmentCopy.$userID !== _RT.$topUser.$userID && !_UI.pMain.pSearch.oIncludeYourEdits.NODISPLAY && _UI.pMain.pSearch.oIncludeYourEdits.CHECKED) return false;
  if (!_UI.pMain.pSearch.oIncludeUpdatedBy.NODISPLAY && _UI.pMain.pSearch.oIncludeUpdatedBy.VALUE) {
	...
	if (hash in cache) {
	  ...
	  try {
		...
		if (_RT.$topUser.$isCM && -1 === _RT.$topUser.$countryIDs.indexOf(segmentCopy.$countryID)) return false;
  ...
  if (segmentCopy.$updated && _UI.pMain.pSearch.oIncludeUpdatedSince.VALUE) try {
	  if (!_RT.$includeUpdatedSinceTime) _RT.$includeUpdatedSinceTime = (new Date(_UI.pMain.pSearch.oIncludeUpdatedSince.VALUE)).getTime();
	  if (segmentCopy.$updated < _RT.$includeUpdatedSinceTime) return false
	} catch (e) {
	}
  if (_UI.pMain.pSearch.oIncludeCityName.VALUE) {
	if (!segmentCopy.$cityID) return false;
...
function F_SHOWREPORT(reportFormat) {
  ...
  function getTOC() {
	...
	traverseReport(function(obj) {
	  if (checkFilter(0, obj.$segmentCopy, seenSegments) && getFilteredSeverity(obj.$check.SEVERITY, obj.$checkID, false))
  ...
  function traverseReport(handler) {
	...
	function getSortedSegments(repS) {
	  ...
	  if (!ret || ret.length != repS.$unsortedSegmentIDs.length)
		return repS.$sortedSegmentIDs = [].concat(repS.$unsortedSegmentIDs).sort(function(a, b) {
		...
		for (var sorsid = 0; sorsid < sortedStreets.length; sorsid++) {
		  ...
		  for (var sorscid = 0; sorscid < sortedSegments.length; sorscid++) {
			...
			if (checkID in sc.$reportIDs) {
			  var obj = {$checkID: checkID, $check: check, $param: sc.$reportIDs[checkID], $cityParam: repC.$params[checkID], $streetParam: repS.$params[checkID], $segmentCopy: sc};
			  switch (handler(obj)) {
				case RT_STOP:
				  return;
				case RT_NEXTCHECK:
  ...
  function getReportCheck(obj) {
	...
	FR += getCheckDescription(obj.$checkID, obj.$segmentCopy.$countryID, Bh2, Eh2);
	FR += Br
  }
  ...
  function getReportItem(obj) {
	if (!checkFilter(0, obj.$segmentCopy, seenSegments) || !getFilteredSeverity(obj.$check.SEVERITY, obj.$checkID, false)) return;
	...
	if (obj.$segmentCopy.$cityID !== lastCityID) getReportCity(obj);
	if (obj.$segmentCopy.$streetID !== lastStreetID) getReportStreet(obj);
	lastCheckID = obj.$checkID;
...
function F_VALIDATE(disabledHL) {
  ...
  SimpleRESTRICTION.prototype.getTo = function() {
	return this._to ? this._to : this._to = new SimpleSEGMENT(this.$toID)
  };
  function SimpleSEGMENT(objID) {
	...
	if (classCodeIs(seg, CC_UNDEFINED) || classCodeIs(seg, CC_NULL)) return;
  ...
  SimpleSEGMENT.prototype.getNodeA = function() {
	return this._nodeA ? this._nodeA : this._nodeA = new SimpleNODE(this.$nodeAID, this.$segmentID)
  };
  SimpleSEGMENT.prototype.getNodeB = function() {
	return this._nodeB ? this._nodeB : this._nodeB = new SimpleNODE(this.$nodeBID, this.$segmentID)
  };
  SimpleSEGMENT.prototype.getCenter = function() {
	if (this._center) return this._center;
	this._center = this.$rawSegment.geometry.bounds.getCenterLonLat().clone().transform(WM.projection, WM.displayProjection);
	this._center.lon = Math.round(this._center.lon * 1E5) / 1E5;
	this._center.lat = Math.round(this._center.lat * 1E5) / 1E5;
	return this._center
  };
  SimpleSEGMENT.prototype.getRestrictions = function() {
	var t;
	return this._restrictions ? this._restrictions : this._restrictions = (t = this, this.$rawSegment.attributes.restrictions.map(function(e) {
  ...
  SimpleSEGMENT.prototype.report = function(params) {
	...
	function getSegmentCopy(ss) {
	  return {
		$segmentID: +ss.$segmentID, $countryID: +ss.$address.$countryID, $cityID: +ss.$address.$cityID, $streetID: +ss.$address.$streetID, $reportIDs: {},
			$updated: ss.$updatedOn ? ss.$rawSegment.attributes.updatedOn : ss.$createdOn ? ss.$rawSegment.attributes.createdOn : 0,
	...
	if (_REP.$maxSeverity < check.SEVERITY)
	  if (checkFilter(check.SEVERITY, segmentCopy, null) && getFilteredSeverity(check.SEVERITY, id, true)) _REP.$maxSeverity = check.SEVERITY;
  ...
  function HLSegment(rawSegment) {
	...
	if (!severity || !segmentCopy || !checkFilter(severity, segmentCopy, null)) return;
	var filteredSeverity = getFilteredSeverityObj(severity, segmentCopy.$reportIDs, true);
	if (!filteredSeverity) return;
	var obj = {$severity: filteredSeverity, $geometry: rawSegment.geometry};
  ...
  function deleteSeenSegment(segmentID) {
	...
	for (var cid in repC) {
	  ...
	  for (var sid in repS) {
		...
		var repSG = repS[sid].$segmentIDs;
		for (var sgid in repSG) {
		  if (!repSG.hasOwnProperty(sgid) || +sgid !== segmentID) continue;
		  var reportIDs = repSG[sgid].$reportIDs;
		  for (var repID in reportIDs) {
			if (!reportIDs.hasOwnProperty(repID)) continue;
  ...
  function updateSegmentProperties(selectedSegments, disabledHL) {
	...
	if (!selectedIssues.length) {
	  if (prop && (_REP.$isLimitPerCheck || skippedSegment)) prop.innerHTML = newProp;
	  return
	}
  ...
  function matchRegExp(checkID, segmentID, expandedString, options) {
	...
	if (options[CO_NUMBER] && 0 < _REP.$debugCounter) {
	  ...
	  _REP.$debugCounter--;
	  log(getMsg(
		  'debug log for segment ' + segmentID + ', check #' + checkID,
  ...
  for (var segmentKey in WMo.segments.objects) {
	...
	if (_RT.$topUser.$userLevel <= rawSegment.attributes.lockRank && rawSegment.attributes.updatedOn && 13989024E5 < rawSegment.attributes.updatedOn) {
	  if (rawSegment.selected) {
		skippedSegment = true;
		if (!DEF_DEBUG) selectedSegments.push(segmentID)
	  }
	...
	if (slowChecks) {
	  if (1 === nodeA.$otherSegmentsLen && DIR_UNKNOWN !== direction && !nodeA.$isPartial && !nodeA.$isUturn && !nodeA.$restrictionsLen && !segment.$restrictionsLen && isLimitOk(36) &&
		...
		if ((!nodeB.$isPartial || !nextNode.$isPartial) && otherSegment.$segmentID !== segmentID && otherSegment.$rawSegment && (1E4 > otherSegment.$length + segmentLen || 1E3 > segmentLen) &&
			...
			deepCompare(otherSegment.$flags, flags) && (DIR_TWO === otherSegment.$direction && DIR_TWO === direction || DIR_TWO !== otherSegment.$direction && DIR_TWO !== direction) &&
			otherSegment.$elevation === elevation && otherSegment.$nodeAID !== nodeBID && otherSegment.$nodeBID !== nodeBID && !otherSegment.$restrictionsLen && !otherNode.$restrictionsLen &&
	  ...
	  if (DIR_UNKNOWN !== direction && !nodeB.$isPartial && 1 === nodeB.$otherSegmentsLen && !nodeB.$isUturn && !nodeB.$restrictionsLen && !segment.$restrictionsLen && isLimitOk(37) &&
		...
		if ((!nodeA.$isPartial || !nextNode.$isPartial) && otherSegment.$segmentID !== segmentID && otherSegment.$rawSegment && 1E4 > otherSegment.$length + segmentLen &&
			...
			otherSegment.$revMaxSpeedUnverified === reverseSpeedUnverified && otherSegment.$type === roadType && otherSegment.$isToll === isToll && otherSegment.$hasRestrictions === hasRestrictions &&
			deepCompare(otherSegment.$flags, flags) && (DIR_TWO === otherSegment.$direction && DIR_TWO === direction || DIR_TWO !== otherSegment.$direction && DIR_TWO !== direction) &&
			otherSegment.$elevation === elevation && otherSegment.$nodeAID !== nodeAID && otherSegment.$nodeBID !== nodeAID && !otherSegment.$restrictionsLen && !otherNode.$restrictionsLen &&
			deepCompare(otherSegment.$alts, alts)) {
		  var loopFound = false;
		  for (var i = 0; i < nextNode.$otherSegmentsLen; i++) {
			var thirdSegment = nextNode.$otherSegments[i];
	  ...
	  if (!nodeA.$isPartial) {
		if (slowChecks && 5 < segmentLen && !nodeA.$otherSegmentsLen && nodeA.$rawNode.geometry.bounds && isLimitOk(107) && address.isOkFor(107)) {
		  ...
		  for (var segKey in WMo.segments.objects) {
			...
			if (RR_TRAIL >= SimpleSEGMENT.prototype.getTypeRank(seg.attributes.roadType)) continue;
